    ...
    try:
        ...
        if debug:
            ...
            logger.error(
                ...
                "https://github.com/danielgatis/dramatiq-postgres/issues/new with full "
                "log.",
            )
    exit(1)
...
def make_argument_parser():
    version = importlib.metadata.version("dramatiq-postgres")
    parser = argparse.ArgumentParser(
        prog="dramatiq-postgres",
    ...
    parser.add_argument(
        ...
        metavar="SCHEMA",
        help=(
            "Alternative database schema for dramatiq-postgres DDL."
    ...
    subparser.add_argument(
        ...
        default="30 days",
        help=dedent("""\
        Max age of rejected messages to keep in queue. Format is Postgres
...
def purge_command(args):
    with transaction(args.pool) as curs:
        ...
        curs.execute(BROKER_QUERIES.PURGE_RESULTS)
        expired = curs.rowcount
    logger.info("Deleted %d messages and %d results.", deleted, expired)
...
def init_command(args):
    with transaction(args.pool) as curs:
        created = init(curs, args.schemaname, args.prefix)
    if created:
...
def stats_command(args):
    ...
    for state in "queued", "consumed", "rejected":
